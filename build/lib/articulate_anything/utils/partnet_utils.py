...
def get_obj_ids(dataset_dir):
    ...
    if all(id.isdigit() for id in obj_ids):
        obj_ids.sort(key=int)  # Sort numerically
    else:
        obj_ids.sort()  # Sort alphabetically
    return obj_ids


def extract_obj_type(input_dir):
    if not os.path.exists(join_path(input_dir, "meta.json")):
...
def track_obj_types(dataset_dir="datasets/partnet-mobility-v0/dataset",
    ...
    if rename:
        ...
        if "StorageFurniture" in obj_types:
